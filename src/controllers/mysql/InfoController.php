<?php
    ...
    use MyApp\Models\MySql\InfoModel as modelInfo;
    ...
    class InfoController {
        ...
        public function column(Request $request, Response $response, $arguments, $flag = false) {
            ...
            # Validar y obtener datos del modelo en formato JSON
            if (!$flag && $apiJsonError) { $resultJson = modelInfo::column($queryParams); }
        ...
        public function select(Request $request, Response $response, $arguments, $flag = false) {
            ...
            # Obtener datos del modelo en formato JSON
            if (!$flag) { $validateWhere = modelValidate::whereCount($queryParams); }
            # Decodificar datos del formato JSON y devolver booleano
            if (!$flag) { $apiJsonError = functionJson::jsonError($validateWhere); }
            # Validar y obtener datos del modelo en formato JSON
            if (!$flag && !$apiJsonError) { $resultJson = $validateWhere; }
            # Añadir un error de bandera si no se cumple la validacion anterior
            if (!$flag && !$apiJsonError) { $flag = true; }
            ...
            # Validar y obtener datos del modelo en formato JSON
            if (!$flag && $apiJsonError) { $resultJson = modelInfo::select($queryParams); }
        ...
        public function label(Request $request, Response $response, $arguments, $flag = false) {
            ...
            # Validar y obtener datos del modelo en formato JSON
            if (!$flag && $apiJsonError) { $resultJson = modelInfo::label($queryParams); }
        ...
        public function alias(Request $request, Response $response, $arguments, $flag = false) {
            ...
            # Validar y obtener datos del modelo en formato JSON
            if (!$flag && $apiJsonError) { $resultJson = modelInfo::alias($queryParams); }
        ...
        public function register(Request $request, Response $response, $arguments, $flag = false) {
            ...
            # Validar y obtener datos del modelo en formato JSON
            if (!$flag && $apiJsonError) { $resultJson = modelInfo::register($queryParams); }
