<?php
    ...
    class SyModule {
        ...
        private function assignValues($param, $data) {
            ...
            # Asignar valor del frontend si existe
            if (isset($data['os_table'])) {
                $field = $data['os_table'];
                $this->fdOsTable = $field;
            }
            # Asignar valor del frontend si existe
            ...
            # Asignar valor del frontend si existe
            if (isset($data['sy_icon'])) {
                $field = $data['sy_icon'];
                $this->fdSyIcon = $field;
            }
            # Asignar valor del frontend si existe
            if (isset($data['sy_prefix'])) {
                $field = $data['sy_prefix'];
                $this->fdSyPrefix = $field;
            }
        }
        ...
        public function validate($flag = false) {
            ...
            $getOsTable = $this->getOsTable();
            $getSyEliminate = $this->getSyEliminate();
            $getSyIcon = $this->getSyIcon();
            $getSyPrefix = $this->getSyPrefix();
            ...
            # Obtener mensaje validado
            if (!$flag && !empty($getOsTable) && in_array($param, $alInUp)) {
                $message[] = array('warning' => $getOsTable);
                $flag = true;
            }
            # Obtener mensaje validado
            ...
            # Obtener mensaje validado
            if (!$flag && !empty($getSyIcon) && in_array($param, $alInUp)) {
                $message[] = array('warning' => $getSyIcon);
                $flag = true;
            }
            # Obtener mensaje validado
            if (!$flag && !empty($getSyPrefix) && in_array($param, $alInUp)) {
                $message[] = array('warning' => $getSyPrefix);
                $flag = true;
            }
            # Retornar el mensaje
            return $message;
        ...
        private function getOsName($flag = false) {
            ...
            if (!$flag) {
                # Funciones de validación para el campo del formulario
                $valAlph = $this->fnctAttribute->valAlphabetic($this->fdOsName);
                $isAplha = $this->fnctAttribute->isAlphaAccent($this->fdOsName);
        ...
        private function getOsTable($flag = false) {
            $message = '';
            $minLengh = 5;
            $maxLengh = 255;
            # Mensaje general para retornar en validacion
            $textInfo = 'El campo <b>tabla</b> del formulario';
            # Funciones de validación para el campo del formulario
            $valEmpt = $this->fnctAttribute->isEmpty($this->fdOsTable);
            # Asignar mensaje segun validacion
            if ($valEmpt) { $message .= "$textInfo es obligatorio"; }
            if ($valEmpt) { $flag = true; }
            # Validar si existe informacion
            if (!$flag) {
                # Funciones de validación para el campo del formulario
                $valMin = $this->fnctAttribute->hasMinLength($this->fdOsTable, $minLengh);
                $valMax = $this->fnctAttribute->hasMaxLength($this->fdOsTable, $maxLengh);
                # Asignar mensaje segun validacion
                if (!$flag && $valMin == 0) { $message .= "$textInfo debe tener minimo $minLengh caracteres"; }
                if (!$flag && $valMin == 0) { $flag = true; }
                # Asignar mensaje segun validacion
        ...
        private function getSyIcon($flag = false) {
            $message = '';
            # Mensaje general para retornar en validacion
            $textInfo = 'El campo <b>icono</b> del formulario';
            # Funciones de validación para el campo del formulario
            $valEmpt = $this->fnctAttribute->isEmpty($this->fdSyIcon);
            # Asignar mensaje segun validacion
            if ($valEmpt) { $message .= "$textInfo es obligatorio"; }
            if ($valEmpt) { $flag = true; }
            # Validar si existe informacion
            if (!$flag) {
                # Funciones de validación para el campo del formulario
                $valNum = $this->fnctAttribute->valNumeric($this->fdSyIcon);
                # Asignar mensaje segun validacion
                if (!$flag && $valNum == 0) { $message .= "$textInfo permite solo valores establecidos"; }
                if (!$flag && $valNum == 0) { $flag = true; }
            }
        ...
        private function getSyPrefix($flag = false) {
            $message = '';
            # Mensaje general para retornar en validacion
            $textInfo = 'El campo <b>prefijo</b> del formulario';
            # Funciones de validación para el campo del formulario
            $valEmpt = $this->fnctAttribute->isEmpty($this->fdSyPrefix);
            # Asignar mensaje segun validacion
            if ($valEmpt) { $message .= "$textInfo es obligatorio"; }
            if ($valEmpt) { $flag = true; }
            # Validar si existe informacion
            if (!$flag) {
                # Funciones de validación para el campo del formulario
                $valNum = $this->fnctAttribute->valNumeric($this->fdSyPrefix);
                # Asignar mensaje segun validacion
                if (!$flag && $valNum == 0) { $message .= "$textInfo permite solo valores establecidos"; }
                if (!$flag && $valNum == 0) { $flag = true; }
            }
