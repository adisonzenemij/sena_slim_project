<?php
    ...
    class UsProccess {
        ...
        private function assignValues($param, $data) {
            ...
            # Asignar valor del frontend si existe
            if (isset($data['os_state'])) {
                $field = $data['os_state'];
                $this->fdOsState = $field;
            }
            # Asignar valor del frontend si existe
        ...
        public function validate($flag = false) {
            ...
            # Obtener mensaje validado
            if (!$flag && !empty($getOsState) && in_array($param, $alInUp)) {
                $message[] = array('warning' => $getOsState);
                $flag = true;
            }
            # Obtener mensaje validado
        ...
        private function getOsState($flag = false) {
            $message = '';
            $minLengh = 5;
            $maxLengh = 255;
            # Mensaje general para retornar en validacion
            ...
            # Funciones de validación para el campo del formulario
            $valEmpt = $this->fnctAttribute->isEmpty($this->fdOsState);
            # Asignar mensaje segun validacion
            if ($valEmpt) { $message .= "$textInfo es obligatorio"; }
            if ($valEmpt) { $flag = true; }
            # Validar si existe informacion
            if (!$flag) {
                # Funciones de validación para el campo del formulario
                $valAlph = $this->fnctAttribute->valAlphabetic($this->fdOsState);
                $valMin = $this->fnctAttribute->hasMinLength($this->fdOsState, $minLengh);
                $valMax = $this->fnctAttribute->hasMaxLength($this->fdOsState, $maxLengh);
                # Asignar mensaje segun validacion
