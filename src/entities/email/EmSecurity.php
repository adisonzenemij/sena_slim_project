<?php
    ...
    class EmSecurity {
        ...
        public function validate($flag = false) {
            ...
            # Obtener mensaje validado
            if (!$flag && !empty($getOsType) && in_array($param, $alInUp)) {
                $message[] = array('warning' => $getOsType);
                $flag = true;
            }
            # Obtener mensaje validado
        ...
        private function getOsType($flag = false) {
            $message = '';
            $minLengh = 5;
            $maxLengh = 255;
            # Mensaje general para retornar en validacion
            $textInfo = 'El campo <b>tipo</b> del formulario';
            # Funciones de validación para el campo del formulario
            $valEmpt = $this->fnctAttribute->isEmpty($this->fdOsType);
            # Asignar mensaje segun validacion
            if ($valEmpt) { $message .= "$textInfo es obligatorio"; }
            if ($valEmpt) { $flag = true; }
            # Validar si existe informacion
            if (!$flag) {
                # Funciones de validación para el campo del formulario
                $valAlph = $this->fnctAttribute->valAlphabetic($this->fdOsType);
                $isLower = $this->fnctAttribute->isLowerCase($this->fdOsType);
                $valMin = $this->fnctAttribute->hasMinLength($this->fdOsType, $minLengh);
                $valMax = $this->fnctAttribute->hasMaxLength($this->fdOsType, $maxLengh);
                # Asignar mensaje segun validacion
