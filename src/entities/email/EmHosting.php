<?php
    ...
    class EmHosting {
        ...
        private function assignValues($param, $data) {
            ...
            # Asignar valor del frontend si existe
            if (isset($data['os_service'])) {
                $field = $data['os_service'];
                $this->fdOsService = $field;
            }
            # Asignar valor del frontend si existe
        ...
        public function validate($flag = false) {
            ...
            $getOsService = $this->getOsService();
            ...
            # Obtener mensaje validado
            if (!$flag && !empty($getOsService) && in_array($param, $alInUp)) {
                $message[] = array('warning' => $getOsService);
                $flag = true;
            }
            # Obtener mensaje validado
        ...
        private function getOsService($flag = false) {
            $message = '';
            $minLengh = 5;
            $maxLengh = 255;
            # Mensaje general para retornar en validacion
            $textInfo = 'El campo <b>servicio</b> del formulario';
            # Funciones de validación para el campo del formulario
            $valEmpt = $this->fnctAttribute->isEmpty($this->fdOsService);
            # Asignar mensaje segun validacion
            if ($valEmpt) { $message .= "$textInfo es obligatorio"; }
            if ($valEmpt) { $flag = true; }
            # Validar si existe informacion
            if (!$flag) {
                # Funciones de validación para el campo del formulario
                $valMin = $this->fnctAttribute->hasMinLength($this->fdOsService, $minLengh);
                $valMax = $this->fnctAttribute->hasMaxLength($this->fdOsService, $maxLengh);
                # Asignar mensaje segun validacion
