<?php
    ...
    class ApResource {
        ...
        private function assignValues($param, $data) {
            ...
            # Asignar valor del frontend si existe
            if (isset($data['ap_causal'])) {
                $field = $data['ap_causal'];
                $this->fdApCausal = $field;
            }
            # Asignar valor del frontend si existe
        ...
        public function validate($flag = false) {
            ...
            # Obtener mensaje validado
            if (!$flag && !empty($getApCausal) && in_array($param, $alInUp)) {
                $message[] = array('warning' => $getApCausal);
                $flag = true;
            }
            # Obtener mensaje validado
        ...
        private function getApCausal($flag = false) {
            $message = '';
            # Mensaje general para retornar en validacion
            $textInfo = 'El campo <b>causal</b> del formulario';
            # Funciones de validación para el campo del formulario
            $valEmpt = $this->fnctAttribute->isEmpty($this->fdApCausal);
            # Asignar mensaje segun validacion
            if ($valEmpt) { $message .= "$textInfo es obligatorio"; }
            if ($valEmpt) { $flag = true; }
            # Validar si existe informacion
            if (!$flag) {
                # Funciones de validación para el campo del formulario
                $valNum = $this->fnctAttribute->valNumeric($this->fdApCausal);
                # Asignar mensaje segun validacion
